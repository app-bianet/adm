<?php
  class comprapModel extends MySql{
   ...
   public function GenerarCod($id){
    ...
    public function InsertDt($idproveedor,$idcondpago,$idusuario,$cod_compra,$desc_compra,$numerod,$numeroc, $tipo,
      ...
      try{  
        ...
        //Actualizar el siguiente codigo
        $this->ActCod();
        return $lastId;
      } catch(PDOException $e){
    ...
    public function AddStockArt($idarticulo,$iddeposito,$cantidad,$valor,$tipoa){
      $this->Idarticulo=$idarticulo;
      $this->Iddeposito=$iddeposito;
      $this->Cantidad=$cantidad;
      $this->Valor=$valor;
    ...
    public function AjustarCosto($idarticulo,$costo,$pdesc,$valor){
      $this->Idarticulo=$idarticulo;
      $this->Costo=$costo;
      $this->Pdesc=$pdesc;
      $this->Valor=$valor;
      ...
      try {
        ...
        while ($num_elementos < count($this->Idarticulo)){
          ...
          SET costo=? 
          WHERE idarticulo=?";
          $arrData=array((($this->Costo[$num_elementos]+(($this->Costo[$num_elementos]
          *$this->Pdesc[$num_elementos])/100))/$this->Valor[$num_elementos]),$this->Idarticulo[$num_elementos]);
    ...
    public function AnularProcesarDocumentoImp($origenc,$origend,$totalh){
      ...
      try {
        ...
        saldoh=?,
        fechadb=NOW()
        WHERE idcompra=(SELECT idcompra FROM tbcompra WHERE cod_compra='$this->Origenc' AND tipo='$this->Origend')";
        $arrData=array('Registrado',$this->Totalh);
    ...
    public function ActDetalleImp($idcompraop,$origenc){
      $this->Idcompraop=$idcompraop;
      $this->Origenc=$origenc;
      try {
        $sql="UPDATE tbcomprad SET importado=? WHERE idcompra='$this->Idcompraop'";
        $arrData=array($this->Origenc);
        $request=$this->Update($sql,$arrData);
        return $request;
      } catch (PDOException $e){
        return PDOError($e,'update');
