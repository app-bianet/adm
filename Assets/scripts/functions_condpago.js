...
function Operacion(operacion) {
  switch (operacion) {
    ...
    case "nuevo":
      ...
      $("input[type=textn]").val("0").attr("readonly", false);
      MostrarForm(true);
      break;

    case "mostrar":
      $('[data-toggle="tooltip"]').tooltip();
      $("input[type=text],input[type=textc],input[type=textn]").attr("readonly", true);
    ...
    case "editar":
      $("input[type=text],input[type=textc],input[type=textn]").attr("readonly", false);
      $("#btnEditar").attr("disabled", true);
      $("#btnGuardar,#btnCancelar").attr("disabled", false);
      break;
...
function InsertarEditar() {
  document.addEventListener('submit', function (e) {
    ...
    let strCampo = document.querySelectorAll("#cod_condpago,#desc_condpago,#dias");

    if (empty(strCampo[0].value && strCampo[1].value && strCampo[2].value)) {
      Swal.fire({
        icon: "info",
        title: 'Atención!',
...
function mostrar(idcondpago) {
  ...
  dataset.append('idcondpago', idcondpago);
