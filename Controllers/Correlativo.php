...
class Correlativo extends Controllers{
  ...
  public function Insertar(){
    if(isset($_POST["security"])){
      ...
      if (empty($idcorrelativo)) {
        $resquest=$this->model->InsertDt($cod_correlativo,$desc_correlativo,$grupo,
        $tabla,$cadena,$precadena,$cod_num,$largo);
        $option=1;
      } else {
        $resquest=$this->model->EditarDt($idcorrelativo,$cod_correlativo,$desc_correlativo,$grupo,
        $tabla,$cadena,$precadena,$cod_num,$largo);
        $option=2;
      }
  ...
  public function Activar(){
    if (isset($_POST['idcorrelativo'])) {
      $idcorrelativo=intval(limpiarCadena($_POST['idcorrelativo']));
      $estatus=intval(1);
      $resquest=$this->model->EstatusDt($idcorrelativo,$estatus);
        if($resquest>0){
