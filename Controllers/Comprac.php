<?php
class Comprac extends Controllers{
  ...
  public function comprac(){
    ...
    if (!isset($_SESSION["sidusuario"])){
      ...
      if ($_SESSION['comprac']==1){     
        $data['page_tag']="Cotización de Compra";
        $data['page_title']=".:: Cotización de Compra ::.";
        $data['page_name']="comprac";
        $data['func']="functions_compra.js";
        $this->views->getView($this,"comprac",$data);
      } else {
  ...
  public function Mostrar(){
    if (isset($_POST['idcompra'])) {
      ...
      if ($idcompra){
        ...
        $arrData=array('status'=>false,'msg'=>'No Existen Registros!');
      }
    } else{
      header("Location:".base_URL()."Error404");
    }
  ...
  public function Listar(){
    if (isset($_POST["security"])) {
      $arrData=$this->model->SelectDt('Cotizacion');
      $al='style="text-align:';
      $w='; width:';
      ...
      if($arrData[$i]['estatus']=='Registrado'){
        ...
        '<small '.$al.'center'.$w.'150px;" class="small btn-group">
          ...
          <'.$btn.'warning" onclick="anular('.$arrData[$i]['idcompra'].',\''.$arrData[$i]['totalh'].'\')" '.$dtg.'Anular"><i class="fa fa-exclamation-triangle"></i></button>
          <'.$btn.'danger" onclick="eliminar('.$arrData[$i]['idcompra'].',\''.$arrData[$i]['totalh'].'\')" '.$dtg.'Eliminar"><i class="fa fa-times"></i></button>
