...
class Zona extends Controllers{
  ...
  public function Insertar(){
    if(isset($_POST["security"])){
      ...
      if (empty($idzona)) {
        $resquest=$this->model->InsertDt($cod_zona,$desc_zona);
        $option=1;
      } else {
       $resquest=$this->model->EditarDt($idzona,$cod_zona,$desc_zona,$desc_zona);
        $option=2;
      }
  ...
  public function Activar(){
    if (isset($_POST['idzona'])) {
      $idzona=intval(limpiarCadena($_POST['idzona']));
      $estatus=intval(1);
      $resquest=$this->model->EstatusDt($idzona,$estatus);
        if($resquest>0){
