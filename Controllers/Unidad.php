...
class Unidad extends Controllers{
  ...
  public function Insertar(){
    if(isset($_POST["security"])){
      ...
      if (empty($idunidad)) {
        $request=$this->model->InsertDt($cod_unidad,$desc_unidad);
        $option=1;
      } else {
       $request=$this->model->EditarDt($idunidad,$cod_unidad,$desc_unidad,$desc_unidad);
        $option=2;
      }
  ...
  public function Activar(){
    if (isset($_POST['idunidad'])) {
      $idunidad=intval(limpiarCadena($_POST['idunidad']));
      $estatus=intval(1);
      $request=$this->model->EstatusDt($idunidad,$estatus);
        if($request>0){
